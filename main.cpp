...
int main()
{
  ...
  SwitchPrint sP; // Switch Print class will handle screen out put prompts warnings data

  gS.setFontSize();  // also GameFunction class method will set font size.
  gS.setUpWindow();   // call to GameSpace class GameSpace, which inherits from the GameFunction class, the method will maximize window size.

  sP.printMain(nsSPM::DC::DISPLAY_GREETINGS, gS);  // Switch Print class will handle screen out put prompts warnings data
  ...
  system("cls");      // clear the newly created window   for game space display
  ...
  if (_bContinue = gS.testIfGoalShouldMoveOrResize())    
    ...
    sP.printMain(nsSPM::DC::PLAYERS_DATA_DISPLAY, gS); // Switch Print class will handle screen out put prompts warnings data MOSTLY!
    sP.printMain(nsSPM::DC::GAMES_DATA_DISPLAY, gS);
    sP.printMain(nsSPM::DC::GOALS_DATA_DISPLAY, gS);
   
    sP.printMain(nsSPM::DG::GOALS_ICON_DISPLAY, gS); // Display the GOAL perimeter
    sP.printMain(nsSPM::DG::PLAYERS_ICON_DISPLAY, gS); // Display the PLAYER's location on the 'game space' display
    sP.printMain(nsSPM::DG::GAMES_ICON_DISPLAY, gS); // Display the PLAYER's location on the 'game space' display
...
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++     
    ...
    {
      sP.printMain(nsSPM::DC::GOAL_IN_CONTENTION, gS);
...
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++       
    else if ( gS.get_sWhoLostLastHand() == "PLAYER")
    {
     ...
     // and Z axis movement values and test and or correct them as needed
      ...
      sP.printMain(nsSPM::DC::CLEAR_PLAYERS_XYZ_INPUT, gS);
      sP.placement(nsGF::PLAYER_X_POSITION_PROMPT_COLUMN, nsGF::PLAYER_X_POSITION_PROMPT_ROW);
   ...
   //   gS.testSetGoalPossession("PLAYER");
      ...
      sP.printMain(nsSPM::DC::CLEAR_PLAYERS_XYZ_INPUT, gS);
      sP.printMain(nsSPM::DC::PLAYERS_XYZ_POSITION_DELTAS, gS);
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    else  if (gS.get_sWhoLostLastHand() == "GAME") 
    {
      ...
      gS.displayCards();
      gS.testSetGoalPossession("GAME");
      sP.printMain(nsSPM::DC::GAMES_DATA_DISPLAY, gS);  // Only need this in GAME pick first as PLAYER needs to see GAME’s movement data
      sP.printMain(nsSPM::DG::GAMES_ICON_DISPLAY, gS); // Display the PLAYER's location on the 'game space' display
      sP.printMain(nsSPM::DG::PLAYERS_ICON_DISPLAY, gS); // Display the PLAYER's location on the 'game space' display
      _cQuit = gS.pickPlayersCard();
