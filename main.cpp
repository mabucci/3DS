...
HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE); // Get the console handle  needed for font size and window setup  Windows wants it as globle
...
int main()
{
  ...
  bool _bFirstPlay{ true }; // This will have a random pick for who has to throw-down first.
 ...
 dPW.printMain(nsSPM::DC::DISPLAY_GREETINGS, gS);  // Display Promps Warning class will handle screen out put prompts warnings data

  // if player wants to;  move up and down through greeting, play, or quit option picked up in this while loop

  while (1)
  {
    gS.set_Quit(_getch());    // Hang on player’s input
    if ((int)gS.get_Quit() == nsGF::ESC) /* 27 == esc key code */
    {
      system("cls");   // the cls is here to clear the screen on exit
      return 0; // Exit game.
    }
    else if ((int)gS.get_Quit() != nsGF::ESC) // any other key to play
      break;  // Play game.     
  }// end while(1)
  ...
  gS.displayCards();
  gS.buildGameSpace();
  gS.whoLostLastHand(_bFirstPlay);  // At the start of a match who throws-down first is randomly picked 
 ...
 dPW.printMain(nsSPM::DC::GOALS_DATA_DISPLAY, gS); 
  ...
  { 
    ...
    // Now test if the Goal is in possession, if it is NOT then test if it will make random move and or random size change.
    // If GOAL is in stage two possession then test if GOAL should change size. 
    /*********************************************************************************************************************************************/
  if (_bContinue = gS.testIfGoalShouldMoveOrResize())    
   dPW.printMain(nsSPM::DC::GOALS_DATA_DISPLAY, gS);
 /*****************************************************************************************************************************************************/

    // If an ICON (GAME or PLAYER) move put them in side the GOAL, or that move keeps them in the GOAL update their possession state.
   ...
   dPW.printMain(nsSPM::DG::GAMES_ICON_DISPLAY, gS); // Display the GAME's location on the 'game space' display   
...
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++     
    if (gS.get_baGamesPossessionState(0) && gS.get_baPlayersPossessionState(0))
    {
     dPW.printMain(nsSPM::DC::GOAL_IN_CONTENTION, gS);
      ...
      gS.testGoalContentionWinner(); // there is no dedicated kickOut method   the kick out to new position is done in  gS.testGoalContentionWinner();
