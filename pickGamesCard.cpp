...
void GameFunctions::pickGamesCard()
{
  ...
  // this code is in the above for loop, it just send GAME heading to the closes corner of the GOAL
  // it will be replaced by the code below to move GAME to the closes part of GOAL perimeter
  /***************************************************************************************************************************/
...
*****/
  ...
  if (_bGoalInContention)  // start of “if else” logic to determine next best move for GAME 
  {
      ...
      {  //  If you get here then GAME can not beat PLAYER in contention over GOAL so find lest valuable GAME card and play that and by that give up the GOAL
        for (int i{ nsGF::MAX_NUMBER_OF_GAME_CARDS - 2 }; i >= 0; --i)
        {
          ...
          {  // If that card is found select it as GAME trump card.
            ...
            _iGamesTrumpCardValue = _iaGamesTrumpCard[0] + 1 + _iaGamesTrumpCard[1] + 1;
            _LbGamesTrumpCardFound = true;  // _L* prefix indicates local variable.
            break; // break out of for loop
          }
        }
        // find the card with a value that will beat PLAYER in throw-down in GOAL contention
        // GAME does not have to use it highest card value just one that will beat PLAYER’s highest  
      }else if (_LiPlayersHigestCardValue <= _LiGamesHighestCardValue) 
      {
        for (int i{ nsGF::MAX_NUMBER_OF_GAME_CARDS - 2 }; i >= 0; --i)
        {
          if (_iaGamesCards[i][0] == -1) // Work up from the bottom of GAME’s card list.
            continue;
          if (_iaGamesCards[i][0] + 1 + _iaGamesCards[i][1] + 1 > _LiPlayersHigestCardValue)
          {  // If that card is found select it as GAME trump card.
        ...
        }// end of  if on "(_LiPlayersHigestCardValue < _LiGamesHighestCardValue)"

    }// end if on  "(_iPlayersLongestPathTotal >= _LiPlayersHigestCardValue && _iGamesLongestPathTotal >= _LiGamesHighestCardValue)"
  ...
  else if (_LbGameCanMoveInGoal == false && _baGamesPossessionState[0] == false && _baGamesPossessionState[1] == false )    // Start of if/else change 
  {
    ...
    if (_LbGamesTrumpCardFound)
      for (int i{ 0 }; i < nsGF::NUMBER_OF_DIMENSIONS; ++i)
      {
        ...
        _iaGamesRequestedMove[i] = std::round(_LdGamesPercnetsOfXYZDeltaToGoalsPerimeter[i] * _iGamesTrumpCardValue);
...
* 
* Data needed to perform the OODA;  PLAYER’s and GAME’s position, GOAL’s current dimensions. 
* Advantage to bumping the PLAYER, if GAME runs our of room gets into a corner say, and jumping over PLAYER would put PLAYER in a 
* ++position then bumping is the best option. 
* Disadvantage to bumping the PLAYER, if GAME won last hand then PLAYER has to throw-down card first and move to PLAYER’s new position, but if GAME lost 
* last hand then GAME has to throw-down first and move to new position, this will make it easy for PLAYER to move to a ++position 
* (a good position from which to jump into the GOAL.) 
* Advantage to out running the PLAYER, if GAME has cards with significant high movement values then running a way with the GOAL will 
* leave the PLAYER with little choice but to pursue while using up valuable cards.  
* If GAME can keep GOAL out of PLAYER jump range until cards exhausted, or GOAL size change hit 1,1,1, GAME will win match. 
* Disadvantage GAME must have and use-up many if not all if its’ high movement value cards. 
* Advantage of inviting a GOAL contention, there isn’t any.     GAME can run and it can’t bump, its last hope is to get into a contention 
* loop where it and PLAYER contend over the GOAL until cards used up or GOAL sizes to 1,1,1. and match end, hopefully with GAME in possession of GOAL.
* Disadvantage of inviting a GOAL contention, see advantage.
*/
  ...
  // else to top if... 
   else if (playersGoalPossession[0] == true && playersGoalPossession[1] == false)
